...
class NutritionalStatusWFAReportGeneration extends Controller
{
    private static $categories = ['Normal', 'Underweight', 'Severely Underweight', 'Overweight'];
    private static $categoryKeys = ['normal', 'underweight', 'severely_underweight', 'overweight'];
    ...
    public static function generate($userId, $cdcId = 0)
    {
        ...
        $reportId = DB::table('ns_wfa_reports')->insertGetId([
            'user_id' => $userId,
    ...
    public static function generatePDF($userId)
    {
        ...
        $report = DB::table('ns_wfa_reports')
            ->where('user_id', $userId)
        ...
        if (!$report) {
            throw new \Exception('No pending NS Weight-for-Age report found for this user.');
        }
        ...
        try {
            ...
            $reportData = DB::table('ns_wfa_report_data')
            ...
            $pdf->SetTitle('Consolidated Nutritional Status Report - Weight for Age');
            $pdf->SetSubject('NS Weight-for-Age Report');
            ...
            foreach ($reportData as $row) {
                // Build row data - widths match header definitions
                $html .= '<tr>';
                $html .= '<td width="2%">' . $counter++ . '</td>';
                $html .= '<td width="9%">' . mb_strtoupper(htmlspecialchars($row->center_name, ENT_QUOTES, 'UTF-8')) . '</td>';
                $html .= '<td width="9%">' . mb_strtoupper(htmlspecialchars($row->worker_name ?? 'No Worker Assigned', ENT_QUOTES, 'UTF-8')) . '</td>';
                ...
                // Category columns (4 categories × 8 cols = 32 cols)
                ...
                // Total columns (M and F by age) - 8 cols
                foreach ($sexLabels as $sex) {
                    foreach ($ages as $age) {
                        $html .= '<td width="1.875%">' . $rowTotalsBySex[$sex][$age] . '</td>';
            ...
            $html .= '<p style="font-size:7px; text-align:left;">SFP Forms 4.1 (c/o SFP Focal Person)</p>';
            ...
            $fileName = "NS_Weight_For_Age_" . now()->format('m_d_Y_H_i_s') . ".pdf";
    ...
    private static function fetchAndStoreData($reportId, $cycleId, $centerIDs, &$totalRecords)
    {
        ...
        $categoryKeys = ['normal', 'underweight', 'severely_underweight', 'overweight'];
        $ages = [2, 3, 4, 5];
        ...
        foreach ($centers as $center) {
            ...
            foreach ($children as $child) {
                ...
                $wfa = $child->weight_for_age;

                // Map the weight_for_age value to category key
                $catKeyIndex = array_search($wfa, $categories);
            ...
            DB::table('ns_wfa_report_data')->insert(array_merge([
                'report_id' => $reportId,
     ...
     *
     * Column width distribution for WFA (4 categories, must total 100%):
     * - No.: 2%
     * - CDC Name: 9%
     * - CDW Name: 9%
     * - Total Served: 3%
     * - M/F totals: 4% (2% each)
     * - 4 Categories × 8 cols = 32 data columns @ ~1.8% each = ~58%
     * - Total section: 8 cols @ ~1.875% each = ~15%
     *
     * Using table-layout: fixed forces TCPDF to respect these widths.
     */
        ...
        $html = '<table border="1" cellpadding="1" cellspacing="0" style="font-size:5px; border-collapse:collapse; table-layout:fixed;" width="100%">
            <thead>
                <tr style="background-color:#f0f0f0; font-weight:bold;">
                    ...
                    <th rowspan="3" width="9%">Name of Child Development Center</th>
                    <th rowspan="3" width="9%">Name of Child Development Worker</th>
                    <th rowspan="3" width="3%">Total Number Served</th>
                    <th rowspan="2" colspan="2" width="4%">Total No of CDC/SNP Served</th>';

        // 4 categories × 8 columns each = 32 cols, plus Total section (8 columns) = 40 cols
        // Each category block gets ~14.5%, Total gets ~15%
        foreach ($categories as $category) {
            $html .= '<th colspan="8" width="14.5%">' . htmlspecialchars($category) . '</th>';
        }
        $html .= '<th colspan="8" width="15%">Total</th>';
        $html .= '</tr>';
    ...
    private static function buildTotalRows($grandTotals, $categories, $categoryKeys, $ages, $sexLabels)
    {
        ...
        foreach ($categoryKeys as $catKey) {
            foreach ($sexLabels as $sex) {
                $sexStr = $sex === 'M' ? 'M' : 'F';
                foreach ($ages as $age) {
                    $html .= '<td>' . $grandTotals['categories'][$catKey][$sexStr][$age] . '</td>';
                }
            }
        }
        ...
        foreach ($categoryKeys as $catKey) {
            foreach ($sexLabels as $sex) {
                ...
                $catSexTotal = 0;
                foreach ($ages as $age) {
                    $catSexTotal += $grandTotals['categories'][$catKey][$sexStr][$age];
                }
