...
class UndernourishedAfter120ReportGeneration extends Controller
{
    public static function generateReport($userId, $cdcId = 0)
    {
        ...
        $reportId = DB::table('undernourished_after_120_reports')->insertGetId([
            'user_id' => $userId,
        ...
        try {
            ...
            DB::table('undernourished_after_120_reports')
                ->where('id', $reportId)
                ->update([
                    'status' => 'pending',
    ...
    public static function generatePDF($userId)
    {
        ...
        $report = DB::table('undernourished_after_120_reports')
            ->where('user_id', $userId)
        ...
        if (!$report) {
            throw new \Exception('No pending undernourished after 120 report found for this user.');
        }
        ...
        try {
            ...
            $reportData = DB::table('undernourished_after_120_report_data')
            ...
            $pdf->SetTitle('Summary of Undernourished Children After 120 Feedings');
            $pdf->SetSubject('Undernourished Children After 120 Feedings Report');
            ...
            $fileName = "Undernourished_After_120_" . now()->format('m_d_Y_H_m_s') . ".pdf";
            ...
            DB::table('undernourished_after_120_report_data')
                ->where('report_id', $report->id)
                ->delete();
    ...
    private static function fetchAndStoreData($reportId, $cycleId, $centerIDs, &$totalRecords)
    {
        ...
        foreach ($centers as $center) {
            ...
            // Get children who have undernourished nutritional statuses at this center
            ...
            // For "after 120", we need the SECOND nutritional status record (exit/after 120 feedings).
            foreach ($childIds as $childId) {
                $exitNs = DB::table('nutritional_statuses')
                ...
                // Age bracket based on exit NS
                if (in_array($exitNs->age_in_years, [2, 3, 4, 5])) {
                    $counts["age_{$exitNs->age_in_years}_{$sexKey}"]++;
                }
                ...
                // Deworming & Vitamin A from exit NS record
                if ($exitNs->deworming_date != null) {
                    $counts["dewormed_{$sexKey}"]++;
                }
                if ($exitNs->vitamin_a_date != null) {
                    $counts["vitamin_a_{$sexKey}"]++;
                }
            }

            // Only store if there's data for this center
            $hasData = false;
            ...
            DB::table('undernourished_after_120_report_data')->insert(array_merge([
                'report_id' => $reportId,
