...
class NutritionalStatusHFAReportGeneration extends Controller
{
    private static $categories = ['Normal', 'Stunted', 'Severely Stunted', 'Tall'];
    private static $categoryKeys = ['normal', 'stunted', 'severely_stunted', 'tall'];
    ...
    public static function generate($userId, $cdcId = 0)
    {
        ...
        $reportId = DB::table('ns_hfa_reports')->insertGetId([
            'user_id' => $userId,
    ...
    public static function generatePDF($userId)
    {
        ...
        $report = DB::table('ns_hfa_reports')
            ->where('user_id', $userId)
        ...
        if (!$report) {
            throw new \Exception('No pending NS Height-for-Age report found for this user.');
        }
        ...
        try {
            ...
            $reportData = DB::table('ns_hfa_report_data')
            ...
            $pdf->SetTitle('Consolidated Nutritional Status Report - Height for Age');
            $pdf->SetSubject('NS Height-for-Age Report');
            ...
            $html .= '<p style="font-size:7px; text-align:left;">SFP Forms 4.2 (c/o SFP Focal Person)</p>';
            ...
            $fileName = "NS_Height_For_Age_" . now()->format('m_d_Y_H_i_s') . ".pdf";
    ...
    private static function fetchAndStoreData($reportId, $cycleId, $centerIDs, &$totalRecords)
    {
        ...
        foreach ($centers as $center) {
            ...
            foreach ($children as $child) {
                ...
                $hfa = $child->height_for_age;

                $catKeyIndex = array_search($hfa, self::$categories);
            ...
            DB::table('ns_hfa_report_data')->insert(array_merge([
                'report_id' => $reportId,
