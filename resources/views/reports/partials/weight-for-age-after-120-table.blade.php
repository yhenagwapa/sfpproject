<table id='weight-for-age-after-120-table' class="table datatable mt-3 text-xs text-center">
    <thead class="border bg-gray-200">
        <tr>
            <th class="border border-white w-30" rowspan="3">No.</th>
            <th class="border border-white w-30" rowspan="3">Name of Child Development Center</th>
            <th class="border border-white w-30" rowspan="3">Name of Child Development Worker</th>
            <th class="border border-white w-10" rowspan="3">Total Number Served</th>
            <th class="border border-white w-10" rowspan="2" colspan="2">Total No of CDC/SNP Served</th>
            <th class="border border-white" colspan="8">Normal</th>
            <th class="border border-white" colspan="8">Underweight</th>
            <th class="border border-white" colspan="8">Severly Underweight</th>
            <th class="border border-white" colspan="8">Overweight</th>
            <th class="border border-white" colspan="8">Total</th>
       
        <tr>
            <th class="border border-white w-2" colspan="4">Male</th>
            <th class="border border-white w-2" colspan="4">Female</th>
            <th class="border border-white w-2" colspan="4">Male</th>
            <th class="border border-white w-2" colspan="4">Female</th>
            <th class="border border-white w-2" colspan="4">Male</th>
            <th class="border border-white w-2" colspan="4">Female</th>
            <th class="border border-white w-2" colspan="4">Male</th>
            <th class="border border-white w-2" colspan="4">Female</th>
            <th class="border border-white w-2" colspan="4">Male</th>
            <th class="border border-white w-2" colspan="4">Female</th>
        </tr>
        <tr>
            <th class="border border-white">M</th>
            <th class="border border-white">F</th>
            <th class="border border-white">2</th>
            <th class="border border-white">3</th>
            <th class="border border-white">4</th>
            <th class="border border-white">5</th>
            <th class="border border-white">2</th>
            <th class="border border-white">3</th>
            <th class="border border-white">4</th>
            <th class="border border-white">5</th>
            <th class="border border-white">2</th>
            <th class="border border-white">3</th>
            <th class="border border-white">4</th>
            <th class="border border-white">5</th>
            <th class="border border-white">2</th>
            <th class="border border-white">3</th>
            <th class="border border-white">4</th>
            <th class="border border-white">5</th>
            <th class="border border-white">2</th>
            <th class="border border-white">3</th>
            <th class="border border-white">4</th>
            <th class="border border-white">5</th>
            <th class="border border-white">2</th>
            <th class="border border-white">3</th>
            <th class="border border-white">4</th>
            <th class="border border-white">5</th>
            <th class="border border-white">2</th>
            <th class="border border-white">3</th>
            <th class="border border-white">4</th>
            <th class="border border-white">5</th>
            <th class="border border-white">2</th>
            <th class="border border-white">3</th>
            <th class="border border-white">4</th>
            <th class="border border-white">5</th>
            <th class="border border-white">2</th>
            <th class="border border-white">3</th>
            <th class="border border-white">4</th>
            <th class="border border-white">5</th>
            <th class="border border-white">2</th>
            <th class="border border-white">3</th>
            <th class="border border-white">4</th>
            <th class="border border-white">5</th>
        </tr>
    </thead>
    <tbody class="weight-for-age-after-120-table text-xs">
        @php
            $count = 0;

            $totalServed = 0;
            $totalMale = 0;
            $totalFemale = 0;

            $normalAged2Male = 0;
            $underweightAged2Male = 0;
            $severelyUnderweightAged2Male = 0;
            $overweightAged2Male = 0;

            $normalAged2Female = 0;
            $underweightAged2Female = 0;
            $severelyUnderweightAged2Female = 0;
            $overweightAged2Female = 0;

            $normalAged3Male = 0;
            $underweightAged3Male = 0;
            $severelyUnderweightAged3Male = 0;
            $overweightAged3Male = 0;

            $normalAged3Female = 0;
            $underweightAged3Female = 0;
            $severelyUnderweightAged3Female = 0;
            $overweightAged3Female = 0;

            $normalAged4Male = 0;
            $underweightAged4Male = 0;
            $severelyUnderweightAged4Male = 0;
            $overweightAged4Male = 0;

            $normalAged4Female = 0;
            $underweightAged4Female = 0;
            $severelyUnderweightAged4Female = 0;
            $overweightAged4Female = 0;
            
            $normalAged5Male = 0;
            $underweightAged5Male = 0;
            $severelyUnderweightAged5Male = 0;
            $overweightAged5Male = 0;

            $normalAged5Female = 0;
            $underweightAged5Female = 0;
            $severelyUnderweightAged5Female = 0;
            $overweightAged5Female = 0;

            $totalAged2Male = 0;
            $totalAged3Male = 0;
            $totalAged4Male = 0;
            $totalAged5Male = 0;

            $totalAged2Female = 0;
            $totalAged3Female = 0;
            $totalAged4Female = 0;
            $totalAged5Female = 0;
        @endphp

        @foreach ($centers as $center)
            @php
                $count += 1;

                $totalServed += $totals[$center->id]['total_served'] ?? 0;
                $totalMale += $totals[$center->id]['total_male'] ?? 0;
                $totalFemale += $totals[$center->id]['total_female'] ?? 0;

                $normalAged2Male += $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['normal']['male'] ?? 0;
                $normalAged3Male += $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['normal']['male'] ?? 0;
                $normalAged4Male += $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['normal']['male'] ?? 0;
                $normalAged5Male += $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['normal']['male'] ?? 0;

                $normalAged2Female += $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['normal']['female'] ?? 0;
                $normalAged3Female += $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['normal']['female'] ?? 0;
                $normalAged4Female += $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['normal']['female'] ?? 0;
                $normalAged5Female += $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['normal']['female'] ?? 0;

                $underweightAged2Male += $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['underweight']['male'] ?? 0;
                $underweightAged3Male += $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['underweight']['male'] ?? 0;
                $underweightAged4Male += $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['underweight']['male'] ?? 0;
                $underweightAged5Male += $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['underweight']['male'] ?? 0;

                $underweightAged2Female += $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['underweight']['female'] ?? 0;
                $underweightAged3Female += $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['underweight']['female'] ?? 0;
                $underweightAged4Female += $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['underweight']['female'] ?? 0;
                $underweightAged5Female += $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['underweight']['female'] ?? 0;

                $severelyUnderweightAged2Male += $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['severely_underweight']['male'] ?? 0;
                $severelyUnderweightAged3Male += $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['severely_underweight']['male'] ?? 0;
                $severelyUnderweightAged4Male += $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['severely_underweight']['male'] ?? 0;
                $severelyUnderweightAged5Male += $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['severely_underweight']['male'] ?? 0;

                $severelyUnderweightAged2Female += $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['severely_underweight']['female'] ?? 0;
                $severelyUnderweightAged3Female += $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['severely_underweight']['female'] ?? 0;
                $severelyUnderweightAged4Female += $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['severely_underweight']['female'] ?? 0;
                $severelyUnderweightAged5Female += $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['severely_underweight']['female'] ?? 0;

                $overweightAged2Male += $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['overweight']['male'] ?? 0;
                $overweightAged3Male += $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['overweight']['male'] ?? 0;
                $overweightAged4Male += $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['overweight']['male'] ?? 0;
                $overweightAged5Male += $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['overweight']['male'] ?? 0;

                $overweightAged2Female += $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['overweight']['female'] ?? 0;
                $overweightAged3Female += $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['overweight']['female'] ?? 0;
                $overweightAged4Female += $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['overweight']['female'] ?? 0;
                $overweightAged5Female += $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['overweight']['female'] ?? 0;

                $totalAged2Male += $totals[$center->id]['2']['male'] ?? 0;
                $totalAged3Male += $totals[$center->id]['3']['male'] ?? 0;
                $totalAged4Male += $totals[$center->id]['4']['male'] ?? 0;
                $totalAged5Male += $totals[$center->id]['5']['male'] ?? 0;

                $totalAged2Male += $totals[$center->id]['2']['female'] ?? 0;
                $totalAged3Female += $totals[$center->id]['3']['female'] ?? 0;
                $totalAged4Female += $totals[$center->id]['4']['female'] ?? 0;
                $totalAged5Female += $totals[$center->id]['5']['female'] ?? 0;

            @endphp
            <tr>
                <td>{{ $count }}</td>
                <td>{{ $center->center_name }}</td>
                <td>{{ $center->user->full_name }}</td>
                <td>{{ $totals[$center->id]['total_served'] ?? 0 }}</td>
                <td>{{ $totals[$center->id]['total_male'] ?? 0 }}</td>
                <td>{{ $totals[$center->id]['total_female'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['normal']['male'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['normal']['male'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['normal']['male'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['normal']['male'] ?? 0 }}</td>

                <td>{{ $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['normal']['female'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['normal']['female'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['normal']['female'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['normal']['female'] ?? 0 }}</td>

                <td>{{ $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['underweight']['male'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['underweight']['male'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['underweight']['male'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['underweight']['male'] ?? 0 }}</td>

                <td>{{ $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['underweight']['female'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['underweight']['female'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['underweight']['female'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['underweight']['female'] ?? 0 }}</td>

                <td>{{ $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['severely_underweight']['male'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['severely_underweight']['male'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['severely_underweight']['male'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['severely_underweight']['male'] ?? 0 }}</td>

                <td>{{ $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['severely_underweight']['female'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['severely_underweight']['female'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['severely_underweight']['female'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['severely_underweight']['female'] ?? 0 }}</td>

                <td>{{ $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['overweight']['male'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['overweight']['male'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['overweight']['male'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['overweight']['male'] ?? 0 }}</td>

                <td>{{ $ageGroupsPerCenter[$center->id]['2']['weight_for_age']['overweight']['female'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['3']['weight_for_age']['overweight']['female'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['4']['weight_for_age']['overweight']['female'] ?? 0 }}</td>
                <td>{{ $ageGroupsPerCenter[$center->id]['5']['weight_for_age']['overweight']['female'] ?? 0 }}</td>

                <td>{{ $totals[$center->id]['2']['male'] ?? 0 }}</td>
                <td>{{ $totals[$center->id]['3']['male'] ?? 0 }}</td>
                <td>{{ $totals[$center->id]['4']['male'] ?? 0 }}</td>
                <td>{{ $totals[$center->id]['5']['male'] ?? 0 }}</td>

                <td>{{ $totals[$center->id]['2']['female'] ?? 0 }}</td>
                <td>{{ $totals[$center->id]['3']['female'] ?? 0 }}</td>
                <td>{{ $totals[$center->id]['4']['female'] ?? 0 }}</td>
                <td>{{ $totals[$center->id]['5']['female'] ?? 0 }}</td>
            </tr>
        @endforeach

        <tr>
            <td class="text-right" colspan="3">Total per Age Bracket ></td>
            <td rowspan="3">{{ $totalServed }}</td>
            <td>{{ $totalMale }}</td>
            <td>{{ $totalFemale }}</td>

            <td>{{ $normalAged2Male }}</td>
            <td>{{ $normalAged3Male }}</td>
            <td>{{ $normalAged4Male }}</td>
            <td>{{ $normalAged5Male }}</td>
            <td>{{ $normalAged2Female }}</td>
            <td>{{ $normalAged3Female }}</td>
            <td>{{ $normalAged4Female }}</td>
            <td>{{ $normalAged5Female }}</td>

            <td>{{ $underweightAged2Male }}</td>
            <td>{{ $underweightAged3Male }}</td>
            <td>{{ $underweightAged4Male }}</td>
            <td>{{ $underweightAged5Male }}</td>
            <td>{{ $underweightAged2Female }}</td>
            <td>{{ $underweightAged3Female }}</td>
            <td>{{ $underweightAged4Female }}</td>
            <td>{{ $underweightAged5Female }}</td>

            <td>{{ $severelyUnderweightAged2Male }}</td>
            <td>{{ $severelyUnderweightAged3Male }}</td>
            <td>{{ $severelyUnderweightAged4Male }}</td>
            <td>{{ $severelyUnderweightAged5Male }}</td>
            <td>{{ $severelyUnderweightAged2Female }}</td>
            <td>{{ $severelyUnderweightAged3Female }}</td>
            <td>{{ $severelyUnderweightAged4Female }}</td>
            <td>{{ $severelyUnderweightAged5Female }}</td>

            <td>{{ $overweightAged2Male }}</td>
            <td>{{ $overweightAged3Male }}</td>
            <td>{{ $overweightAged4Male }}</td>
            <td>{{ $overweightAged5Male }}</td>
            <td>{{ $overweightAged2Female }}</td>
            <td>{{ $overweightAged3Female }}</td>
            <td>{{ $overweightAged4Female }}</td>
            <td>{{ $overweightAged5Female }}</td>

            <td>{{ $totalAged2Male }}</td>
            <td>{{ $totalAged3Male }}</td>
            <td>{{ $totalAged4Male }}</td>
            <td>{{ $totalAged5Male }}</td>
            <td>{{ $totalAged2Female }}</td>
            <td>{{ $totalAged3Female }}</td>
            <td>{{ $totalAged4Female }}</td>
            <td>{{ $totalAged5Female }}</td>
            
        </tr>

        @php
            $totalServedMaleFemale = 0;

            $allNormalMale = 0;
            $allNormalFemale = 0;

            $allUnderweightMale = 0;
            $allUnderweighteFemale = 0;

            $allSeverelyUnderweightMale = 0;
            $allSeverelyUnderweightFemale = 0;

            $allOverweightMale = 0;
            $allOverweightFemale = 0;

            $allMale = 0;
            $allFemale = 0;

            $allNormal = 0;
            $allUnderweight = 0;
            $allSeverelyUnderweight = 0;
            $allOverweight = 0;
            $allTotal = 0;


            $totalServedMaleFemale = $totalMale + $totalFemale;

            $allNormalMale = $normalAged2Male + $normalAged3Male + $normalAged4Male + $normalAged5Male;
            $allNormalFemale = $normalAged2Female + $normalAged3Female + $normalAged4Female + $normalAged5Female;

            $allUnderweightMale = $underweightAged2Male + $underweightAged3Male + $underweightAged4Male + $underweightAged5Male;
            $allUnderweighteFemale = $underweightAged2Female + $underweightAged3Female + $underweightAged4Female + $underweightAged5Female;

            $allSeverelyUnderweightMale = $severelyUnderweightAged2Male + $severelyUnderweightAged3Male + $severelyUnderweightAged4Male + $severelyUnderweightAged5Male;
            $allSeverelyUnderweightFemale = $severelyUnderweightAged2Female + $severelyUnderweightAged3Female + $severelyUnderweightAged4Female + $severelyUnderweightAged5Female;

            $allOverweightMale = $overweightAged2Male + $overweightAged3Male + $overweightAged4Male + $overweightAged5Male;
            $allOverweightFemale = $overweightAged2Female + $overweightAged3Female + $overweightAged4Female + $overweightAged5Female;

            $allMale =  $totalAged2Male + $totalAged3Male + $totalAged4Male + $totalAged5Male;
            $allFemale = $totalAged2Female + $totalAged3Female + $totalAged4Female + $totalAged5Female;

            $allNormal = $allNormalMale + $allNormalFemale;
            $allUnderweight = $allUnderweightMale + $allUnderweighteFemale;
            $allSeverelyUnderweight = $allSeverelyUnderweightMale + $allSeverelyUnderweightFemale;
            $allOverweight = $allOverweightMale + $allOverweightFemale;
            $allTotal = $allMale + $allFemale;
        @endphp
        
        <tr>
            <td class="text-right" colspan="3">Total Male/Female ></td>
            <td rowspan="2" colspan="2">{{ $totalServedMaleFemale }}</td>
            <td colspan="4">{{ $allNormalMale }}</td>
            <td colspan="4">{{ $allNormalFemale }}</td>
            <td colspan="4">{{ $allUnderweightMale }}</td>
            <td colspan="4">{{ $allUnderweighteFemale }}</td>
            <td colspan="4">{{ $allSeverelyUnderweightMale }}</td>
            <td colspan="4">{{ $allSeverelyUnderweightFemale }}</td>
            <td colspan="4">{{ $allOverweightMale }}</td>
            <td colspan="4">{{ $allOverweightFemale }}</td>
            <td colspan="4">{{ $allMale }}</td>
            <td colspan="4">{{ $allFemale }}</td>
        </tr>

        <tr>
            <td class="text-right" colspan="3">Total Child Beneficiaries ></td>
            <td colspan="8">{{ $allNormal }}</td>
            <td colspan="8">{{ $allUnderweight }}</td>
            <td colspan="8">{{ $allSeverelyUnderweight }}</td>
            <td colspan="8">{{ $allOverweight }}</td>
            <td colspan="8">{{ $allTotal }}</td>
        </tr>
    </tbody>
</table>

