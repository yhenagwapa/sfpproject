        ...
        @if ($children)
            @foreach ($children as $child)
                <tr>
                    ...
                    <td>
                        {{-- @dd($child->records) --}}
                        <select class="form-control uppercase w-full border-none"
                        @if ($child->records->first()?->status == 'dropped')
                            disabled
                        @endif>
                            <option value="" disabled>Select status</option>
                            <option value="active" {{ $child->records->first()?->status === 'active' ? 'selected' : '' }}>
                                Active
                            </option>
                            {{-- <option value="transferred" {{ $child->records->first()?->status === 'transferred' ? 'selected' : '' }}>
                                Transferred
                            </option> --}}
                            <option value="dropped" {{ $child->records->first()?->status === 'dropped' ? 'selected' : '' }}>
                                Dropped
                            </option>
