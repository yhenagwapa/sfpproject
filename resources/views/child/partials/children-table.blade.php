    ...
    <tbody class="text-sm">
        @if ($children)
            @foreach ($children as $child)
                @php
                    /// uncomment if pangit na doble makita na student
                    // if (auth()->user()->hasAnyRole(['admin', 'lgu focal', 'sfp coordinator'])){
                    //     $records = collect([$child->records->last()])->filter();

                    // } else{
                    //     $records = $child->records->sortBy('created_at');
                    // }

                    $records = $child->records->sortByDesc('created_at');
                @endphp
                    ...
                    <tr>
                        ...
                        <td>
                            <select id="statusSelect-{{ $child->id }}" name="status"
                                class="
                                    @if ($records->first()?->action_type === 'dropped' || $records->first()?->action_type === 'transferred')
                                        rounded border-gray-300 bg-gray-500 text-white uppercase w-full
                                    @else
                                        rounded border-gray-300 uppercase w-full
                                    @endif
                                    "
                                @if ($records->first()?->action_type === 'dropped' || $records->first()?->action_type === 'transferred') disabled @endif
                            >

                                <option value="" selected disabled>{{ $records->first()?->action_type }}</option>

                                @if ($records->first()?->action_type == 'transferred' || $records->first()?->action_type == 'active')
                                ...
                                @if (!auth()->user()->hasAnyRole(['child development worker', 'encoder']))
                                    @if ($records->first()?->action_type === 'transferred')
                                    ...
                                    @endif
                                @elseif ($records->first()?->action_type === 'transferred')
                                    <option value="transferred" disabled>
                                        Transferred
                                    </option>
                                @endif

                                @if ($records->first()?->action_type === 'dropped')
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <select id="selectedCenter{{ $child->id }}" class="form-control rounded border-gray-300 uppercase"
                                            ...
                                            @foreach ($centerNames as $center)
                                                @if ($center->id != $records->first()?->centerTo?->id)
                        ...
                        <form id="statusForm-{{ $child->id }}" method="POST" action="{{ route('child.update-status') }}">
                            ...
                            <input type="hidden" name="oldCenter"
                                value="{{ $records->first()?->centerTo?->id ?? $records->first()?->centerFrom?->id }}">
                        ...
                        <script>
                            ...
                            let originalStatus{{ $child->id }} = "{{ $records->first()?->action_type }}";
                            ...
                            <div class="flex space-x-3">
                                ...
                                @if (!auth()->user()->hasAnyRole(['lgu focal', 'sfp coordinator']))
                                    @if (session('temp_can_edit') || auth()->user()?->can('edit-child'))
                                        @if ($records->first()?->action_type != 'dropped' || auth()->user()->hasRole('admin'))
                                            @if ($child->edit_counter != 2)
                                                <form id="editChild-{{ $child->id }}" action="{{ route('child.show') }}"
                                                    ...
                                                    <input type="hidden" name="child_id" value="{{ $child->id }}">
                                                    <input type="hidden" name="child_status" value="{{ $records->first()?->action_type }}">
